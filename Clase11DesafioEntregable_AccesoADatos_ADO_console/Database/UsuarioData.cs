...
namespace Clase11DesafioEntregable_AccesoADatos_ADO_console.Database
{
    public static class UsuarioData
    {
        ...
        public static List<Usuario> ListarUsuarios()
        {
            ...
            {
                throw new Exception("Usuarios no Encontrados", ex);
            }
        }
        ...
        public static void ModificarVenta(Usuario usuario, int id)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "UPDATE Venta SET Nombre = @nombre, Apellido = @apellido, NombreUsuario = @nombreUsuario WHERE Id = @id";
                    SqlCommand command = new SqlCommand(query, connection);
